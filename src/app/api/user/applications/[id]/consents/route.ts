...
import { appNotFound, adminPermissionDenied, internalError, paramInvalid, tokenRevokeFailed } from '@/lib/api-response';
...
export async function GET(
  ...
  try {
    ...
    // Non-admin users can only view consents for their own apps
    ...
    console.error('User get app consents error:', error);
...
export async function DELETE(
  ...
  try {
    ...
    // Non-admin users can only revoke consents for their own apps
    if (user.role !== 'admin' && client.userId !== user.id) {
      return appNotFound();
    }
    ...
    console.error('User revoke app consent error:', error);
